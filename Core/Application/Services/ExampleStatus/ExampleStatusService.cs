...
{
    public class ExampleStatusService : BaseService, IExampleStatusService
    {
        #region Fields
        private readonly IExampleStatusRepository _repository;
        #endregion

        #region Constructor
        public ExampleStatusService(IExampleStatusRepository repository, IMapper mapper, IUserInfoService userInfoService) : base(mapper, userInfoService)
        {
        ...
        public async Task<ExampleStatusInfoDto> GetInfoByIdAsync(int id, EnumLanguage lang)
        {
        ...
        {
            var filter = new ExampleStatusFilter
            {
                
            };
            return await GetAllAsLookupAsync(filter);
        }
        public async Task<List<LookupDto>> GetAllAsLookupAsync(ExampleStatusFilter filter)
        {
            var list = await GetAllAsync(filter);
            return list.Select(e => new LookupDto(e.ExampleStatusId,e.Name)).ToList();
        }
        #endregion
    }
}
