    ...
    */
    $.fn.smkValidate = function () {
        ...
        if (this.is('form')) {
            ...
            if (this[0].hasAttribute("without-validation")) {
                return true;
            }
        }
        // Se inicializan las variables globales
        var result = true;
        var filesResult = true;

        // Se obtienen todos los inputs
        var inputs = (this.is(':input') || this.is('select') || this.is('checkbox')) ? $(this) : $(':input:visible:not(:button):not(:disabled):not(.novalidate):not(.select2-search__field):not(.runtime-form):not(.has-business-error):not(.without-validation)', this);
        var inputValid = true;
        // Se recorren todos los inputs del formulario
        inputs.each(function (k, v) {
            // Se obtiene el objeto input
            inputValid = $.validateInput(v);
        ...
        var files = (this.is(':input[type="file"]')) ? $(this) : $(':input[type="file"]:visible:not(:disabled):not(.novalidate):not(.runtime-form):not(.runtime):not(.has-business-error):not(.without-validation)', this);
        if (files.length > 0) {
            filesResult = $.validateFiles(files);
            if (result && !filesResult) {
                result = false;
            }
        }
    ...
    $.validateFiles = function ($files) {
        ...
        $files.each(function (index) {
            filesIsvalid = validationHelper.validateFile($(this));
            if (!filesIsvalid) {
                return false;
            }
        });
    ...
    $.validateInput = function (v) {
        ...
        if (languaje === undefined) {
            ...
            languaje = {
                ...
                textEmptyCustom: '{@} is required',
                textEmail: 'Please enter a valid email address',
                textAlphanumeric: 'Only letters and numbers are allowed',
                textEngilsh: 'Please enter a valid English data',
                textArabic: 'Please enter a valid Arabic data',
                textNumber: 'Only numbers are allowed',
                textNumberRange: 'The allowed numbers must be greater than <b> {@} </b> and less than <b> {@} </b>',
                ...
                textSPassStrong: 'Minimum 6 characters including a number and a capital letter',
                ...
                textPattern: 'Please enter a valid data',
                textCrNumber: 'Please enter a valid foundation number',
                textFoundationNumber: 'Please enter a valid foundation number',
                textDomain: 'Please enter a valid domain',
                textDomainURL: 'Please enter a valid domain URL',
                textPostalCode: 'Please enter a valid postal code',
                textMobile: 'Please enter a valid mobile number',
                textPhone: 'Please enter a valid phone number',
                textExPhone: 'Please enter a valid external phone number',
                textIdNumber: 'Please enter a valid ID number',
                textNationalId: 'Please enter a valid National ID number',
                textResidencenumber: 'Please enter a valid residence (Iqama) number'
            };
        }
        ...
        else if ($(v)[0].nodeName.toLocaleLowerCase() === 'select') {

            value = $(v).find(":selected").attr('value');
        }
        // Se validan los INPUTS que son requeridos y estan vacios
        // add input type multible validation 
        if ((required === 'required') && (value === undefined || value.trim() === '' || value.trim().length === 0)) {
            
            // Se agrega el mensaje de error
            var customeRequiredMsg = input.attr("smk-error-msg-required");
            if (customeRequiredMsg === undefined || customeRequiredMsg === '') {
                let label = $(v).parent('div').find('label').text().trim();
                if (label !== undefined || label !== '') {
                    customeRequiredMsg = $.smokeCustomizeMsg(languaje.textEmptyCustom, label);
                }
                else {
                    customeRequiredMsg = languaje.textEmpty;
                }
            }
            result = $.smkAddError(input, customeRequiredMsg);
            /* if (this.type.indexOf("multiple") > -1) {
                // Se agrega el mensaje de error
                var customeRequiredMsg = input.attr("smk-error-msg-required");
                if (customeRequiredMsg === undefined || customeRequiredMsg === '') {
                    customeRequiredMsg = languaje.textEmpty;
        ...
        if (value !== '') {
            ...
            if (type === 'number' || smkType === 'number') {
                ...
                if (!numberRegex.test(value)) {
                    ...
                    if ((typeof (smkMin) !== 'undefined' && typeof (smkMax) !== 'undefined')) {
                        smkMax = parseInt(smkMax);
                        smkMin = parseInt(smkMin);
            ...
            // Cr Number
            else if (type === 'crnumber' || smkType === 'crnumber') {
                // Se crea la expresión regular para el input decimal^[2-9]\d{9}$
                //var decimalRegex = /^\d+(?:\.\d{1,4})?$/;
                var crnumberRegex = /^[1-9]\d{9}$/;
                // Se valida que el value del input cumpla con la expresión regular
                if (!crnumberRegex.test(value)) {
                    // Se agrega el mensaje de error
                    result = $.smkAddError(input, languaje.textCrNumber);
                }
            }
            //English
            else if (type === 'englishtext' || smkType === 'englishtext') {
                //let regex = /^[a-zA-Z0-9$@$!%*?؟&#^-_.,""“”;:؛،()'\r\n\\/ +]+$/;
                //if (!regex.test(value)) {
                //    // Se agrega el mensaje de error
                //    result = $.smkAddError(input, languaje.textEngilsh);
                //}
            }
            //Arabic
            else if (type === 'arabictext' || smkType === 'arabictext') {
                //let regex = /^[\u0621-\u064A\u0660-\u06690-9$@$!%*?؟&#^-_.\\/,""“”;:؛،ٌُ()'ً\ٍَِr\n +]*$/;
                //if (!regex.test(value)) {
                //    // Se agrega el mensaje de error
                //    result = $.smkAddError(input, languaje.textArabic);
                //}
            }

            // foundation Number
            else if (type === 'foundationnumber' || smkType === 'foundationnumber') {
                var foundationnumberRegex = /^[7]\d{10}$/;
                if (!foundationnumberRegex.test(value)) {
                    result = $.smkAddError(input, languaje.textFoundationNumber);
                }
            }
            
            // domain
            else if (type === 'domain' || smkType === 'domain'){
                var domainRegex = /[a-zA-Z0-9][a-zA-Z0-9-]{1,61}[a-zA-Z0-9](?:\.[a-zA-Z]{2,})+/;
                if (!domainRegex.test(value)) {
                    result = $.smkAddError(input, languaje.textDomain);
                }
            }

            // PostalCode Number
            else if (type === 'postalcode' || smkType === 'postalcode' ) {
                // Se crea la expresión regular para el input decimal^[2-9]\d{9}$
                //var decimalRegex = /^\d+(?:\.\d{1,4})?$/;
                var postalcodeRegex = /^[1-9]\d{4}$/;
                // Se valida que el value del input cumpla con la expresión regular
                if (!postalcodeRegex.test(value)) {
                    // Se agrega el mensaje de error
                    result = $.smkAddError(input, languaje.textPostalCode);
                }
            }
            // Mobile Number
            else if (type === 'mobile' || smkType === 'mobile') {
                // Se crea la expresión regular para el input decimal^[2-9]\d{9}$
                //var decimalRegex = /^\d+(?:\.\d{1,4})?$/;
                var mobileRegex = /^[0][5]\d{8}$/;
                // Se valida que el value del input cumpla con la expresión regular
                if (!mobileRegex.test(value)) {
                    // Se agrega el mensaje de error
                    result = $.smkAddError(input, languaje.textMobile);
                }
            }

            // Phone Number
            else if (type === 'phone' || smkType === 'phone') {
                var mobileRegex = /^[01]\d{9}$/;
                // Se valida que el value del input cumpla con la expresión regular
                if (!mobileRegex.test(value)) {
                    // Se agrega el mensaje de error
                    result = $.smkAddError(input, languaje.textPhone);
                }
            }
            // External Phone Number
                ...
                if (!mobileRegex.test(value)) {
                    // Se agrega el mensaje de error
                    result = $.smkAddError(input, languaje.textExPhone);
                }
            }
            // IdNumber 
            else if (type === 'idnumber' || smkType === 'idnumber') {
                // Se crea la expresión regular para el input decimal^[2-9]\d{9}$
                //var decimalRegex = /^\d+(?:\.\d{1,4})?$/;
                var idnumberRegex = /^[1-2]\d{9}$/;
                // Se valida que el value del input cumpla con la expresión regular
                if (!idnumberRegex.test(value)) {
                    // Se agrega el mensaje de error
                    result = $.smkAddError(input, languaje.textIdNumber);
                }
            }
            // IdNumber 
            else if (type === 'nationalid' || smkType === 'nationalid') {
                // Se crea la expresión regular para el input decimal^[2-9]\d{9}$
                //var decimalRegex = /^\d+(?:\.\d{1,4})?$/;
                var nationalidRegex = /^[1]\d{9}$/;
                // Se valida que el value del input cumpla con la expresión regular
                if (!nationalidRegex.test(value)) {
                    // Se agrega el mensaje de error
                    result = $.smkAddError(input, languaje.textNationalId);
                }
            }
            // ResidenceNumber
            else if (type === 'residencenumber' || smkType === 'residencenumber') {
                // Se crea la expresión regular para el input decimal^[2-9]\d{9}$
                //var decimalRegex = /^\d+(?:\.\d{1,4})?$/;
                var residenceRegex = /^[2]\d{9}$/;
                // Se valida que el value del input cumpla con la expresión regular
                if (!residenceRegex.test(value)) {
                    // Se agrega el mensaje de error
                    result = $.smkAddError(input, languaje.textResidencenumber);
                }
            }
            // Se valida el input DECIMAL
            else if (smkType === 'decimal') {
            ...
            if (type === 'url') {
                ...
                if (!urlRegex.test(value)) {
                    ...
                    var customeRequiredMsg = input.attr("smk-error-msg-url");
                    if (customeRequiredMsg === undefined || customeRequiredMsg === '') {
                        customeRequiredMsg = languaje.textUrl;
                    }
                    result = $.smkAddError(input, customeRequiredMsg);
            ...
            if (smkPattern !== '' && smkPattern !== undefined) {
                ...
                if (!patternRegex.test(value)) {
                    ...
                    var customeRequiredMsg = input.attr("smk-error-msg-pattern");
                    if (customeRequiredMsg === undefined || customeRequiredMsg === '') {
                        customeRequiredMsg = languaje.textPattern;
                    }
                    result = $.smkAddError(input, customeRequiredMsg);
