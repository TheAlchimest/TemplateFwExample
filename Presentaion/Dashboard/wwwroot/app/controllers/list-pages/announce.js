var announceListController = {
    emptyAction:'<span class="btn btn-light text-white btn-md py-1 d-none">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>',
    activateAction: '<button onclick="announceListController.activate(this);" class="btn btn-outline-success btn-md py-1"> تفعيل </button>',
    deactivateAction: '<button onclick="announceListController.deactivate(this);"  class="btn btn-outline-danger btn-md py-1"> إيقاف </button>',
    editAction: '<a href="#!/announce/edit/{id}" class="btn btn-outline-primary btn-md py-1">تعديل</a>',
    deleteAction: '<button onclick="announceListController.delete(this);" class="btn btn-outline-danger btn-md py-1">حذف</button>',
    titleTd: '<span class="text-primary"><b>{title}</b></span>',
    activStatus: '<span  class="btn btn-success btn-md py-1 btn-block"> مفعل </span>',
    ...
    drawRowActions: function ($tr) {
        ...
        ctrl.drawActivationAction($tr, isDefault);
        ctrl.drawEditAction($tr, activationCount);
        ctrl.drawDeleteAction($tr, isDefault);
        ctrl.drawTitle($tr);
    ...
    drawActivationAction: function ($tr, isDefault) {
    ...
    drawEditAction: function ($tr, activationCount) {
        ...
        let ctrl = this;
        let action = ctrl.editAction.replace("{id}", $tr.attr("data-id-encode"));
        $tr.find(".td-edit").html(action);
    },
    drawDeleteAction: function ($tr, isDefault) {
        
        let ctrl = this;
        let action = (!isDefault) ? ctrl.deleteAction : ctrl.emptyAction;
    ...
    activate: function (element) {
        ...
        ctrl.confirm($tr, operationTypes.activate, '/announce/activate');
    },
    deactivate: function (element) {
        
        let ctrl = this;
        let $tr = ctrl.getActionTr(element);
        ctrl.confirm($tr, operationTypes.deactivate, '/announce/deactivate');
    ...
    delete: function (element) {
        ...
        let ctrl = this;
        let $tr = ctrl.getActionTr(element);
        ctrl.confirm($tr, operationTypes.delete, '/announce/delete', function () {
